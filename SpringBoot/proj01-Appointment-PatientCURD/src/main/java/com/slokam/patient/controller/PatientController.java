...
public class PatientController {
	...
	@GetMapping("getByAddr/{name}")
	public ResponseEntity<List<Patient>> findByAddress(@PathVariable String name){
	
		 List<Patient> list=  patientRepo.findByAddr(name); 
		return new ResponseEntity<List<Patient>>(list, HttpStatus.OK);
	}
	
	@GetMapping("getPatientByName/{sesha}")
	public ResponseEntity<List<Patient>> getPatientByName(@PathVariable("sesha") String name){
		List<Patient> list= patientRepo.getPatientByName("%"+name+"%");
		/*
		 * if(list.isEmpty()) { return new
		 * ResponseEntity<List<Patient>>(HttpStatus.NOT_FOUND) ; }
		 */
		return new ResponseEntity<List<Patient>>(list,HttpStatus.OK);
	}
